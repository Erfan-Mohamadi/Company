            ...
            ->columns([
                ...
                TextColumn::make('value')
                    ...
                    ->formatStateUsing(function ($state, Setting $record): string {
                        if (blank($state)) {
                            return '<span class="text-gray-500 italic">' . __('Not set') . '</span>';
                        }

                        return match ($record->type) {
                            Setting::TYPE_IMAGE => '<img src="' . e($state) . '" style="max-height: 80px; max-width: 120px; border-radius: 6px; object-fit: contain;" alt="' . __('Image preview') . '" loading="lazy" />',

                            Setting::TYPE_VIDEO => '<video src="' . e($state) . '" style="max-height: 80px; max-width: 120px; border-radius: 6px;" controls muted preload="metadata"></video>',

                            Setting::TYPE_CHECKBOX => (filter_var($state, FILTER_VALIDATE_BOOLEAN) || $state === '1')
                            ...
                            Setting::TYPE_TEXTAREA => Str::limit(nl2br(e($state)), 30),
